...
def register_employer(employer_data: EmployerRegister, db: Session = Depends(get_db)):
    if db.query(Employer).filter(Employer.email == employer_data.email.lower()).first():
        raise HTTPException(status_code=400, detail="Email already registered")
    
    if db.query(Employer).filter(Employer.phone_number == employer_data.phone_number).first():
        raise HTTPException(status_code=400, detail="Phone number already registered")
...
def login(login_data: Login, db: Session = Depends(get_db)):
    ...
    elif login_data.user_type == "employer":
        employer = db.query(Employer).filter(Employer.email == email).first()
        if not employer or not verify_password(login_data.password, employer.hashed_password):
            raise HTTPException(status_code=401, detail="Invalid credentials")
        user_id = employer.id
    else:
        raise HTTPException(status_code=400, detail="Invalid user type")
    
    token_data = {"sub": user_id, "user_type": login_data.user_type}
